...
private:
    ...
    void trocar(ItemListaDeNos* a, ItemListaDeNos* b)
    {
        ...
        */
        No* aux = a->getItem();
        float auxPontos = a->getPontuacaoNoCluster();
    ...
    bool oMinimoDosClustersFoiAtingido(float lowerBound[], int nClusters)
    {
        ...
        {
            if(clusters[i]->getPontuacaoAtual()<lowerBound[i])
            {
                return false;
            }
        }
    ...
    void insereItemNaSolucaoPelosPontos(int index, int nClusters)
    {
        ...
        this->limpaClones(itemEscolhido,nClusters);
        clusters[clusterEscolhido]->adicionaNo(listaDeCandidatos->popNo(index), clusterEscolhido);
...
public:
    ...
    void geraSolucao(int nClusters, Grafo* g, float L[], float U[], float alpha)
    {
    ...
    ListaDeNos** solucaoGulosoRandomizado(int nClusters, Grafo* g, float L[], float U[], int iteracoes, float alpha)
    {
    ...
    ListaDeNos** solucaoGulosoRandomizadoReativo(int nClusters, Grafo* g, float L[], float U[], int iteracoes, int iterEntreAtualizacoes, int alphas[], float fatorAmplificacao)
    {
        ...
        while(iterAtual<iteracoes)
        {
            ...
            for(int j = 0; j<iterEntreAtualizacoes; j++)
            {
                ...
                {
                    geraSolucao(nClusters,g,L,U,alphas[i]);
